...
dump_80211(const u_char *packet, unsigned plen)
{
	...
	if (fcf.type == 2 && fcf.subtype == 8) {
		print_mac(src); printf(" --> "); print_mac(dst); putchar('\n');
	}

	// if (fcf.type == QOS_DATA) { //only find associated clients
...
main(int argc, char **argv)
{
	...
	/* Setup filter to only capture data types - this makes sure client 
	is associated with AP
	*/
	struct bpf_program filter;

	if (pcap_compile(handle, &filter, "type data", 0, 0) == -1) {
        printf("Bad filter - %s\n", pcap_geterr(handle));
        return 2;
    }
	...
	pcap_loop(handle, count ,pkt_callback,NULL);
